...
object BinaryInterpreterTests {
    ...
    @Test fun intArray() {
        ...
        val cvt = LittleEndian.IntArrayInter(intArray.size)
    ...
    @Test fun float() {
        ...
        val cvt = LittleEndian.FloatInter
        cases.forEach { f ->
    ...
    @Test fun floatArray() {
        ...
        val cvt = LittleEndian.FloatArrayInter(floatArray.size)
    ...
    @Test fun uByteInter() {
        val cvt = LittleEndian.UByteInter
        for (i in (0..255)) {
            assertEquals(i, cvt.interpret(cvt.convert(i)))
        }
    }
    ...
    @Test fun shortInter() {
        val cvt = LittleEndian.ShortInter
    ...
    @Test fun uShortInter(){
        val cvt = LittleEndian.UShortInter
        for(i in 0..65535) {
            assertEquals(i, cvt.interpret(cvt.convert(i)))
        }
    }
