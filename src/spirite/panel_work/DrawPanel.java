...
//		this.addMouseMotionListener(mouser);
    ...
    public void paint( Graphics g) {
        ...
        if( context.master == null) return; // Needed so the IDE doesn't freak out

        // Draw Line
        Graphics2D g2 = (Graphics2D) g;

        // Draw Image
        BufferedImage image = context.master.getImageManager().getActivePart().getData();
        if( image != null) {
            ...
            context.itsY(0)-1,
            (int)Math.round(image.getWidth()*context.zoom)+1,
            (int)Math.round(image.getHeight()*context.zoom)+1);
