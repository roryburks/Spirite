	...
	implements MWorkspaceObserver, MNodeSelectionObserver, MAnimationStateObserver
{
	...
	private final FFAAnimationState animState;
	...
	public FFAnimationSchemePanel( MasterControl master, AnimationNode node) {
		...
		this.animState = (FFAAnimationState)animation.getContext().getAnimationManager().getAnimationState(animation);
	...
	private void BuildFromAnimation() {
		...
		for( int col=0; col < layers.size(); ++col) {
			...
			for(FFAFrame frame : layer.getFrames()) {
	...
	private void _doAction( String command) {
		switch( command) {
		case "duplicate": {
			ws.duplicateNode(((FFAFrame)cmenuObject).getNode());
			break;}
		case "insertEmptyBefore":{
            FFAFrame f = (FFAFrame)cmenuObject;
		...
		case "insertEmpty":{
			...
			break;}
		case "deleteNode":{
			ws.removeNode(((FFAFrame)cmenuObject).getNode());
			break;}
		...
		case "removeGapAfter":{
			...
			break;}
		case "localLoop":{
            //FFAFrame f = (Frame)cmenuObject;
			//f.getLayerContext().wrapInLoop(f);
			break;}
		case "refer":{
			LayerNode ln = ((LayerNode)((FFAFrame)cmenuObject).getNode());
		...
		case "resizeLocalLoop":{
            FFAFrame frame = ((FFAFrame)cmenuObject);
			...
			if( dia.success) {
				if( dia.inLoops) {
                    ...
                    FFAFrame frameIt = frame.getNext();
	...
	private void _openContextMenuForFrame(FFALayer layer, int tick, Point p) {
        FFAFrame frame = layer.getFrameForMet(tick, true);
		_openContextMenuForFrame( frame, p);
	}
	private void _openContextMenuForFrame(FFAFrame frame, Point p) {
		contextMenu.removeAll();
		cmenuObject = frame;
		cmenuPoint = p;
				...
				if( e.getButton() == MouseEvent.BUTTON3) {
					...
					(new Timer( HOLD_DELAY, new ActionListener() { @Override public void actionPerformed(ActionEvent e) {
						if( !rcConsumed) {
                            ...
                            animState.putSubstateForRelativeTick( relTick, dialog.getResult());
							timer = null;
						}
					}}));
					timer.start();
				...
				ws.getUndoEngine().doAsAggregateAction(() -> {
					...
					for( FFAFrame frame : layer.getFrames()) {
						if(frame.getNode() != null)
							frame.getNode().getRender().setVisible(visible);
					}
	...
	private class SoFFramePanel extends SPanel {
		...
		private final FFAFrame frame;
		private final int column;

		SoFFramePanel( FFAFrame frame, int column) {
			this.frame = frame;
			this.column = column;
	...
	private class EoFFramePanel extends SPanel {
		...
		EoFFramePanel( FFAFrame frame, int column) {
			this.frame = frame;
			this.column = column;
			this.setBackground(Color.ORANGE);
	...
	private class ResizingLoopState extends State {
		...
		ResizingLoopState( int c, int s, int e, FFAFrame sofFrame)
	...
	private class FrameFramePanel extends SPanel {
		...
		private final SPanel drawPanel = new SPanel() {
			...
			protected void paintComponent(Graphics g) {
				...
				if( frame.getMarker() == Marker.FRAME) {
					ImageBI img = (ImageBI)master.getRenderEngine().accessThumbnail(frame.getNode(), ImageBI.class);
		...
		private FrameFramePanel( FFAFrame frame, int column) {
			this.frame = frame;
			this.column = column;
		...
		protected void paintComponent(Graphics g) {
			...
			else if( animState.getSelectedFrame() == frame)
				setBackground( Color.YELLOW);
			else if( ws.getSelectedNode() == frame.getNode())
			...
			NillFramePanel(boolean isAfter) {
				...
				this.addMouseListener( new MouseAdapter() {
					...
					public void mousePressed(MouseEvent e) {
						...
						animState.setSelectedMet(TickAtY(p.y));
					}
				});
			}
				...
				if( e.getButton() == MouseEvent.BUTTON1) {
					if( state == null) {
						if( e.getY() <  DRAG_BORDER && hasLowerExpand())
							...
							animState.setSelectedFrame(frame);
							setState( new DraggingFrameState());
						}
					}

					ws.setSelectedNode(frame.getNode());
		...
		private class DraggingFrameState extends FrameState {
			...
			void EndState() {
				if( target != frame.getStart())
					frame.getLayerContext().moveFrame(frame, frame.getLayerContext().getFrameForMet(target, false), target >= frame.getStart());
