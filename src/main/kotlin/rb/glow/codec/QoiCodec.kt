...
class QoiCodec : IImageCodec{
    override fun encode(data: CodecImageData): ByteArray {
                ...
                if( pa == a)
                {
                    ...
                    if( dg >= -32 && dg <= 31 )
                    {
                        ...
                        if( lumR >= -8 && lumR <= 7 && lumB >= -8 && lumB <= 7) {
                            // QOI_OP_LUMA
                            val codeGreen = 0x80 or (dg + 32)
                            val codeLum = ((lumR + 8) shl 4) or ((lumB + 8))
        ...
        val ttl = ttl_run_skips + code_cacheHits + code_deltas + code_lums + code_rgb
        println("Total Bytes: $ttl Runs: $code_runs rgbs: $code_rgb hits: $code_cacheHits deltas: $code_deltas lums: $code_lums /// runttl: $ttl_run_skips")
    ...
    override fun decode(data: ByteArray): CodecImageData {
        ...
        lo@ while( caret < width*height)
        {
            ...
            if( code == 0xFF || code == 0xFE)
            {
                ...
                0x80 -> { // LUMA
                    ...
                    val lumR = ((code2 and 0xf0) shr 4) - 8
                    val lumB = (code2 and 0x0f) - 8
                    val rgba = BitPacking.unpackInt(previousColor)

                    writeColor(
                        // cr = lumR + pr +  cg - pg = lumR + pr + dg
        ...
        println("$@$debug_ttle {len: $debug_fromLen} {fb: $fB} {cache: $fCache} {lum:$flum} {delta: $fdelta}")
