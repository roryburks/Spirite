...
import spirite.base.graphics.gl.fill.GLFillV1.GLFillV1Context
...
interface IFillWorker {
    fun fill(glImage: GLImage, x: Int, y: Int, color: SColor) : IntArray?
}

/***
 * The V1 Fill Algoritm separates the image into 9x9 chunks, consisting of a 1-pixel outer layer (32 pixels) and
 * a 7x7 inner layer (49 pixels; actually could be a 9x11 chunk with 7x9 inner = 63 pixels).
 */
object GLFillV1 : IFillWorker {
    override fun fill(glImage: GLImage, x: Int, y: Int, color: SColor) : IntArray?{
        val w = glImage.width
        val h = glImage.height
        if( x < 0 || x >= w || y < 0 || y >= h) return null
        val existingData = glImage.toIntArray() ?: return null

        val to = color.argb32
        val from = existingData[x + y*h]
        if( to == from) return null

        val context = GLFillV1Context(existingData, w, h, from)
        ...
        val workers =Array((w-1)/9 + 1) {ox ->
            Array( (h-1)/9 + 1) {oy ->
                when {
                    ox == cx && oy == cy -> GLFillV1Worker(context, ox, oy, start = Vec2i(x%9, y%9))
                    else -> GLFillV1Worker(context, ox, oy)
                }
            }
        }
    ...
    class GLFillV1Context(
        ...
        lateinit var workers: Array<Array<GLFillV1Worker>>
        val done: Boolean get() = !workers.any {it.any { it.working }}
    }
}


class GLFillV1Worker(
        val ctx: GLFillV1Context,
        val offsetX: Int,
        val offsetY: Int,
        start : Vec2i? = null
)
