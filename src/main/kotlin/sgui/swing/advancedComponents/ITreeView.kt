...
interface ITreeViewNonUI<T>{
    ...
    interface TreeNodeAttributes<T> : TreeDragInterpreter<T> {
        fun makeComponent( t: T) : ITreeComponent = when( t) {
            ...
            else -> SimpleTreeComponent(SwingComponentProvider.Label(t.toString()))
        }
        ...
        IComponent by swImp
{
    ...
    private fun makeToggleButton( checked: Boolean) : IToggleButton {
        ...
        btn.plainStyle = true
        btn.setOnIcon(SpiriteIcons.SmallIcons.Expanded)
        btn.setOnIconOver(SpiriteIcons.SmallIcons.ExpandedHighlighted)
        btn.setOffIcon(SpiriteIcons.SmallIcons.Unexpanded)
        btn.setOffIconOver(SpiriteIcons.SmallIcons.UnexpandedHighlighted)
        return btn
    }
