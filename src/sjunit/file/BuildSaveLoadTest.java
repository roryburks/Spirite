		...
		if( checkedIImgMap != null  &&checkedIImgMap.containsKey(img1.getId())) {
			...
			if( iimg1 instanceof FlatMedium) {
				...
				for( int i=0; i < colors1.size(); ++i) {
					assert(colors1.get(i).getColor() == colors2.get(i).getColor());
					assert(colors1.get(i).getImg().getXOffset() == colors2.get(i).getImg().getXOffset());
					assert(colors1.get(i).getImg().getYOffset() == colors2.get(i).getImg().getYOffset());
					
					VerifyRawImages(colors1.get(i).getImg().getBase(), colors2.get(i).getImg().getBase());
				}
			}
				...
				for( int i=0; i < things1.size(); ++i) {
					if( things1.get(i) instanceof MagLevStroke) {
						...
						for( int j=0; j<segments1.size(); ++j) {
							assert( segments1.get(j).start == segments2.get(j).start);
							assert( segments1.get(j).strokeId == segments2.get(j).strokeId);
							assert( segments1.get(j).end == segments2.get(j).end);
						}
					}
				}
			}
